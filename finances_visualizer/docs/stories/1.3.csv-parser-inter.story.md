...
```
Data,Lançamento,Valor,Saldo,Descrição,Categoria
15/01/2024,15/01/2024,-50.00,5000.00,"Uber Eats","Alimentação"
16/01/2024,16/01/2024,5000.00,10000.00,"Salário",""
```
...
function detectBankFormat(csvText: string): 'nubank' | 'inter' | 'unknown' {
  ...
  if (headers.includes('data') && (headers.includes('lançamento') || headers.includes('descrição'))) {
    return 'inter';
  }
 ...
 * - Columns: Data, Lançamento, Valor, Saldo, Descrição
 *
...
- [x] Obtain real Inter CSV export sample
- [x] Document column structure
- [x] Note date format differences
- [x] Note amount format differences

### Task 2: Update Format Detection (AC: 1) ✅

- [x] Modify `detectBankFormat()` in `src/lib/csv-parser.ts`
- [x] Add Inter column detection
- [x] Test detection with both formats

### Task 3: Implement Inter Parser (AC: 2, 3, 4, 5) ✅

- [x] Create `parseInterCSV()` function
- [x] Handle Inter date format (DD/MM/YYYY → Date)
- [x] Handle Inter amount format
- [x] Normalize to Transaction[]
- [x] Set `source: 'inter'`

### Task 4: Reuse Validation (AC: 6) ✅

- [x] Use existing `validateTransaction()` from Story 1.2
- [x] Ensure consistent error messages
- [x] Test with Inter samples

### Task 5: Document Format (AC: 7) ✅

- [x] Add JSDoc comments to parser
- [x] Document Inter column structure
- [x] Add examples in comments
- [x] Note assumptions and limitations

### Task 6: Create Tests (Coverage) ✅

- [x] Test Inter format detection
- [x] Test Inter CSV parsing
- [x] Test normalization
- [x] Test with real Inter samples

### Task 7: Update FileUpload Component ✅

- [x] Update to handle both formats
- [x] Show which format was detected
- [x] Test both Nubank and Inter uploads
