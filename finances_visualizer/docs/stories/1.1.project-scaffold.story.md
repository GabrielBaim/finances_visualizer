...
epic: 1
story: 1
title: Project Scaffold and开发环境 Setup
status: Draft
created: 2026-02-03
estimated_days: 1
complexity: Low
...
> **Note**: No architecture documents exist yet. This is the foundation story that establishes the开发环境. All technical decisions come from the PRD Technical Assumptions section.
...
│   └── csv-parser/   # Shared parsing logic (future)
├── apps/
...
- "Minhas finanças nunca foram tão bonitas" tagline
- Tech stack list
...
### Task 1: Initialize Vite + React + TypeScript (AC: 1)

- Run `npm create vite@latest packages/web -- --template react-ts`
- Verify project structure created
- Run `npm install` to install dependencies
- Start dev server to confirm setup works

### Task 2: Configure Tailwind CSS with Dark Theme (AC: 2)
...
- Configure colors in `tailwind.config.js`:
  ...
  ```
- Configure base index.css with Tailwind directives
- Set dark background color as default

### Task 3: Create Folder Structure (AC: 3)

- Create directories: `src/components`, `src/lib`, `src/types`, `src/hooks`
- Create placeholder index files in each (barrel exports)
- Verify Vite can resolve imports from these folders

### Task 4: Setup Vitest with Sample Test (AC: 4)

- Install Vitest: `npm install -D vitest @testing-library/react @testing-library/jest-dom`
- Configure Vitest in `vite.config.ts`
- Create `src/lib/__tests__/sample.test.ts` with basic test
- Run `npm test` to verify passing

### Task 5: Configure Git and Conventional Commits (AC: 5)

- Initialize git: `git init`
- Create initial commit
- (Optional) Install commitlint for conventional commits validation
- Configure `.gitignore`

### Task 6: Create README (AC: 6)

- Create README.md with:
  - Project title: "Visualizador de Finanças"
  - Tagline: "Minhas finanças nunca foram tão bonitas"
  - Tech stack section (React, Vite, TypeScript, Tailwind)
  - Development commands
  - License section

### Task 7: Configure .gitignore (AC: 7)
...
- [ ] Ready for handoff to next story
